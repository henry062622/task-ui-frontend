<template>
      ...
      <a-tabs v-if="tabs.length > 0" v-model:activeKey="activeTab" @change="onTabChange">
        <a-tab-pane v-for="tab in tabs" :key="tab.value" :tab="$t(tab.label)"
          ...
          <div class="flex justify-between items-center !mb-4">
            <div class="flex gap-4 items-center !mb-4" v-if="activeTab == 'all_tasks'">
              ...
              <a-select v-if="currentTab?.filters.includes('website_selection')" v-model:value="filters.site_id"
                :placeholder="$t('selectWebsites')" @change="handleFilter" allow-clear style="width: 220px">
            ...
            <a-select v-if="currentTab?.filters.includes('model_type')" v-model:value="filters.model_type"
              :placeholder="$t('select_model_list')" @change="handleFilter" allow-clear style="width: 220px">
              <a-select-option v-for="site in websiteList" :key="site.id" :value="site.id">
                model{{ site.name }}
              </a-select-option>
            ...
            <a-select v-if="currentTab?.filters.includes('decorative_type')" v-model:value="filters.decorative_type"
              :placeholder="$t('select')" @change="handleFilter" allow-clear style="width: 220px">
            ...
            <div v-else-if="currentTab.value == 'all_tasks' && files.length > 0"
              ...
              <div v-for="file in files" :key="file.storage_url">
                ...
                <div v-else-if="isVideo(file.storage_url)"
                  class="relative w-[120px] aspect-[4/5] flex flex-col gap-4 items-center justify-center border border-gray-200 rounded-lg bg-gray-100 py-4 px-2 cursor-pointer hover:shadow"
                  role="button" tabindex="0" @click="openVideo(file)" @keydown.enter.prevent="openVideo(file)"
                  @keydown.space.prevent="openVideo(file)">
                  <play-circle-outlined class="text-4xl mb-2" />
          ...
          <a-pagination class="!mt-2 flex justify-end" :current="pagination.current" :page-size="pagination.pageSize"
            :total="pagination.total" width="100%" show-quick-jumper @change="handlePageChange" show-less-items />

        </a-tab-pane>
      </a-tabs>
    </div>
    <ModelCreatePopup v-if="showModelPopup" :visible="showModelPopup" :modelTypeList="websiteList" :loading="loading"
      @close="showModelPopup = false" @submit="submitModelCreate" />
  ...
  <!-- Video Preview Modal -->
  <a-modal v-model:open="videoModal.visible" :title="videoModal.title" :footer="null" width="80%" destroy-on-close
    centered @afterClose="resetVideo">
    <video :src="videoModal.src" controls style="width: 100%; height: auto;" />
  </a-modal>
</template>
<script setup>
...
import { FileZipOutlined, PlusOutlined, PlayCircleOutlined } from '@ant-design/icons-vue';
...
const tabs = ref([
  { value: 'all_tasks', label: 'all_tasks', filters: ['file_type', 'website_selection'] },
...
const filters = reactive({ file_type: null, site_id: null, model_type: null, decorative_type: null });
...
function openVideo(file) {
  videoModal.src = file.storage_url
  videoModal.title = file.file_name || 'Video'
  videoModal.visible = true
}

function resetVideo() {
  // stop playback & release any object URLs if you ever use them
  videoModal.src = ''
  videoModal.title = ''
}
  ...
  try {
    const res = await api.get(endpointMap[activeTab.value], {
      params: {
        ...
        website: activeTab.value == 'all_tasks' ? filters.site_id : filters.model_type,
...
const onTabChange = () => {
  ...
  Object.assign(filters, { file_type: null, site_id: null, model_type: null, decorative_type: null });
  fetchFiles(1);
};
