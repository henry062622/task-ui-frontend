...
const onFinish = async (values) => {
  ...
  try {
    await api.post('/api/login', formState.value).then(res => {
      ...
      // If backend returned a token directly, store it and go to dashboard
      if (data?.token) {
      ...
      // Else fallback to legacy 2FA flow
      if (data?.login_token) {
        sessionStorage.setItem('login_token', data.login_token);
        router.push('/two-factory/verify');
      } else {
        // Safety: if neither token nor login_token, treat as error
        loginError.value = 'Unexpected login response.';
      }
    });
  } catch (err) {
    console.log(err);
