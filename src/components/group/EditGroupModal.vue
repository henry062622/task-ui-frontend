<template>
    <a-modal :open="visible" title="Update a Group" :footer="null" :closable="false" centered>
        <a-divider></a-divider>

        <a-form :model="formState" name="update_group" layout="vertical" autocomplete="off" @finish="onSubmit"
...
watch(() => props.visible, (val) => {
    if (val && props.group) {
        formState.value.name = props.group.name
        formState.value.user_ids = props.group.users.map(p => p.id)
        errors.value.name = ''
        fetchUnassignedUserlist();
        // Combine assigned + unassigned users for lookup
...
const onSubmit = async () => {
    ...
    errors.value.name = ''
    try {
        await api.post(`/api/group/${props.group.id}/update`, formState.value)
        emit('updated')
        emit('close')
    } catch (err) {
...
const fetchUnassignedUserlist = async () => {
    ...
    if (props.group) {
        const assignedUsers = props.group.users || []
        allUsers.value = [
            ...allUsers.value,
            ...assignedUsers.filter(assigned => !allUsers.value.some(u => u.id === assigned.id))
        ]
    }
}
